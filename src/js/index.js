...
d3.csv("./cs.csv", function (data) {
    for (var i = 0; i < data.length; i++) {
        var value = parseInt(data[i].value.replace(/ /gi, ""));
        var reallyValue = 0;
        ...
        if (!allArray[data[i].country]) {
            allArray[data[i].country] = {};
            allArray[data[i].country]['value'] = reallyValue;
        }
        else {
            allArray[data[i].country].value += reallyValue;
        }
    ...
    function ready(error, geography, data) {
        ...
        svg
            ...
            .on('mouseover', function (d) {
                var a = '';
                if (allArray[d.properties.name]) {
                    a += d.properties.name + ':' + allArray[d.properties.name].value;
                    $('p').text(a)
                }
                else {
                    a += d.properties.name + ':' + 0;
                    $('p').text(a)
                }
                tip.show(d);
                d3.select(this)
